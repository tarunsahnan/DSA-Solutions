...
public:
   ...
   int rec(vector<int> &prices,int fee,int i,int hold){
       ...
       if(hold == 0){
           int op1 =   rec(prices,fee,i+1,1) - prices[i];
           int op2 =  rec(prices,fee,i+1,0);
       ...
       // STATE OF SELLING THE STOCK
           ...
           int op1 = prices[i] + rec(prices,fee,i+1,0) - fee;
           int op2 = rec(prices,fee,i+1,1);
